...
{
  public:
	RFID(int chipSelectPin, int NRSTPD, int irqPin = -1); // Add optional irq pin
	...
	void halt();
	void enableCardDetectionInterrupt();
	void disableCardDetectionInteqrupt();
	void clearCardDetectionInterrupt();
  ...
  private:
	...
	int _irqPin;
	int _interruptFlag;
	int _lastInterruptTime;

	static RFID* _instance;
	static void handleIRQWrapper();
	void handleIRQ();
};
